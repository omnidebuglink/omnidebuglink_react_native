...
// Close codes
export const CLOSE_CODE_REPLACED = 4000;
export const CLOSE_CODE_PAYLOAD_TOO_LARGE = 1009;

// ---- Native module bridge (OmlReactModule) ----
...
export interface OmlStateResult {
  screen: { width: number; height: number; scale?: number; density?: number };
...
export interface OmlNativeModule {
  ...
  prefsSet(opts: { key: string; value: unknown; valueType: string }): Promise<void>;
  prefsDelete(key: string): Promise<boolean>;
  prefsList(): Promise<{ entries: Record<string, OmlPrefsEntry> }>;
  /**
